...
//ห้องทั้งหมด
$sql_total_room = "SELECT COUNT(*) AS total FROM room";
$result_total_room = $connection->query($sql_total_room);
$total_room = ($result_total_room->num_rows > 0) ? $result_total_room->fetch_assoc()['total'] : 0;

//จองวันนี้
$sql_booking_today = "
    SELECT COUNT(*) AS total 
    FROM booking 
    WHERE book_date = '$selected_date' 
...
$sql_bookstandard_today = "
    ...
    WHERE b.book_date = '$selected_date' 
...
//ห้องว่างวันนี้ = ห้องที่ไม่มีการจองซ้อนกับวันนี้
$sql_available_today = "
    SELECT COUNT(*) AS total 
    FROM room 
    WHERE room_ID NOT IN (
        SELECT room_ID 
        FROM booking 
        WHERE ('$selected_date' BETWEEN check_in AND check_out)
        AND bookstate_ID IN ('B01','B02','B03','B04')
    )
";

$result_available_today = $connection->query($sql_available_today);
$available_today = ($result_available_today->num_rows > 0) ? $result_available_today->fetch_assoc()['total'] : 0;
...
$sql_refund_today = "
    ...
    FROM cancle 
    WHERE cancle_date = '$selected_date'
";
...
$sql_refundstandard_today = "
    ...
    INNER JOIN room r ON b.room_ID = r.room_ID
    INNER JOIN typ_room t ON r.typroom_ID = t.typroom_ID
    WHERE DATE(c.cancle_date) = '$selected_date'
    AND t.typroom_ID = 1
";
...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary" style="font-family: 'Noto Sans Thai', sans-serif;"> <!--begin::App Wrapper-->
    <div class="app-wrapper"> <!--begin::Header-->
        <div class="card-header">
            <div class="col-12">
                <h3 class="mb-0 h3-shadow">รายงาน</h3>
            </div>
        </div>
        <?php
        ...
        <main class="app-main"> <!--begin::App Content Header-->
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <h3 class="mb-0">
                                ...
                                echo formatThaiDate($selected_date);
                                ?>
                            </h3>
                        </div>
                        <!-- ปฏิทินเลือกวันที่ -->
            ...
            <div class="app-content">
                ...
                <div class="container-fluid">
                    <!--begin::Row for Box 1 and 2-->
                    <div class="row">
                        <div class="col-lg-6 col-6">
                            <!-- Small Box Widget 1: ห้องทั้งหมด -->
                        ...
                        <div class="col-lg-6 col-6">
                            <!-- Small Box Widget 2: ห้องว่าง -->
                            <div class="small-box text-bg-success">
                                <div class="inner">
                                    <h3><?php echo $available_today; ?></h3>
                                    <p>ห้องว่าง</p>
                                </div>
                                <!-- SVG icon -->
                            </div>
